...
def remove_missing_values(dataframe):
    """
    Bu fonksiyon, eksik verileri içeren tüm satırları veri setinden çıkarır.
    """
...
def quick_missing_imp_remove(data, num_method=NUM_METHOD, cat_length=CAT_LENGTH, target="price"):
    """
    Bu fonksiyon, eksik verileri içeren satırları siler ve ardından kalan eksik verileri uygun şekilde doldurur.
    """
...
def quick_missing_imp(data, num_method=NUM_METHOD, cat_length=CAT_LENGTH, target="price"):
    variables_with_na = [col for col in data.columns if data[col].isnull().sum() > 0]
    temp_target = data[target]
